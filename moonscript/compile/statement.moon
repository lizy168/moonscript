...

{
  ...
  annotation: (node) =>
    @add node[2], node[-1]
    nil -- @add returns the line, which stm would add a second time

  -- tags existing bindings as constant, has no output. only generated by
  -- transformations, there is no syntax that produces this statement
  declare_constants: (node) =>
    for name in *node[2]
      name = name[2] if ntype(name) == "ref"
      @put_name name, const: true, pos: node[-1]
    nil
