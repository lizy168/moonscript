...
-- moonscript compiler.
--
-- The longer Cfn transform bodies live in moonscript/parse/tree.moon and
-- are required from the callback strings below. The callback strings are
-- Lua source: they are embedded verbatim into the generated parsers.

pgen = require "pgen"
import P, R, S, V, C, Ct, Cc, Cp, Cg, Cmb, Cmt, Cfn, L from pgen

ind = pgen.indenter tab_width: 4, initial: 0
...
dos = pgen.indenter initial: 1
DisableDo = dos.cpush 0
PopDo = dos.pop
CheckDo = dos.ctop "ne", 0
...
Keyword = keyword_patt * -AlphaNum

mark = (name, patt) ->
  Ct Cc(name) * patt

pos = (patt) ->
  Cfn Cp! * patt, [[return function(p, value)
...
-- assignment targets, surfaced from parse() (see moonscript/parse.moon)
assign_transform = [[
  local tree = require("moonscript.parse.tree")
...
op = (chars) ->
  patt = V"Space" * C P chars
  if chars\match "^%w*$"
    patt = patt * -AlphaNum
  patt

key = (chars) ->
  V"Space" * P(chars) * -AlphaNum
